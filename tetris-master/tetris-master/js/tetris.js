...
var colors = [
    '#00f0f0', '#f0a000', '#0000f0', '#f0f000', '#f00000', '#00f000', '#a000f0'
];

// localVars for Ads
const localVars = {
    vState: "ingame_milestone",
    vGameID: "tetris_master_01"
};
...
function clearLines() {
    ...
    for (var y = ROWS - 1; y >= 0; --y) {
        ...
        if (rowFilled) {
            ...
            if (clearsound && (!bgSound || !bgSound.muted)) {
    ...
    if (linesCleared > 0) {
        score += linesCleared * 100 * linesCleared; // Bonus for multiline clears
        updateScoreDisplay();
    }
}
        ...
        for (var x = 0; x < 4; ++x) {
            if (newCurrent[y][x]) {
                if (typeof board[y + offsetY] == 'undefined'
                    ...
                    || y + offsetY >= ROWS
                    || x + offsetX >= COLS) {
                    if (offsetY == 1 && freezed && currentY <= 1) {
